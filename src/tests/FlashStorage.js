(function (window, document, undefined) {
	...
	var _,
  	...
  	QUnit.module("FlashStorage", {
  		beforeEach : function () {
			if ( window.FlashStorage.available ) {
				_ = new window.FlashStorage(function () {
                    _ = undefined;
                }, 3000);
			}
			timer = undefined;
			...
			timer = setInterval(function () {
				if ( _.isLoaded() ) {
					...
					assert.throws(
						...
						function (err) {
							return err.name === "FlashQuotaExceededError";
						},
	...
	QUnit.test("StorageEvent", function (assert) {
		var iframe, done = assert.async(), counter = 0,
            ...
            checkEvent = function (_event) {
                ...
                if ( _event.storage && 
                     ( _event.storage.type === "FlashStorage" ) ) {
                    switch (counter) {
                        case 0:
                            assert.strictEqual(
                                _event.key, 
		...
		if ( _ ) {
			timer = setInterval(function () {
				if ( _.isLoaded() ) {
                    ...
                    iframe.setAttribute("src", "FlashStorage.html"); 
                    appendChild(
