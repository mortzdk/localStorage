(function (window, document, undefined) {
	...
	var _,
  	...
  	QUnit.module("UserDataStorage", {
  		beforeEach : function () {
			if ( window.UserDataStorage.available ) {
			...
			timer = setInterval(function () {
				if ( _.isLoaded() ) {
					...
					assert.throws(
						...
						function (err) {
							return err.name === "UserDataQuotaExceededError";
						},
						"Testing that UserDataStorage throws exception if " + 
                        "data exceeds limit."
					);
	...
	QUnit.test("StorageEvent", function (assert) {
		var iframe, done = assert.async(), counter = 0,
            ...
            checkEvent = function (_event) {
                ...
                if ( _event.storage && 
                     _event.storage.type === "UserDataStorage" ) {
                    switch (counter) {
                        case 0:
                            assert.strictEqual(
                                _event.key, 
		...
		if ( _ ) {
            ...
            iframe.setAttribute("src", "UserDataStorage.html"); 
