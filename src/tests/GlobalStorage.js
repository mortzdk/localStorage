(function (window, document, undefined) {
	...
	var _,
  	...
  	QUnit.module("GlobalStorage", {
  		beforeEach : function () {
			if ( window.GlobalStorage.available ) {
				_ = window.GlobalStorage();
                _.clear();
			}
			timer = null;
  		},
	...
	QUnit.test("StorageEvent", function (assert) {
		var iframe, done = assert.async(), counter = 0,
            ...
            checkEvent = function (_event) {
                ...
                if ( _event.storage && 
                     _event.storage.type === "GlobalStorage" ) {
                    switch (counter) {
                        case 0:
                            assert.strictEqual(
                                _event.key, 
		...
		if ( _ ) {
            ...
            iframe.setAttribute("src", "GlobalStorage.html"); 
