(function (window, document, undefined) {
	...
	var _,
  	...
  	QUnit.module("JavaStorage", {
  		beforeEach : function () {
			if ( window.JavaStorage.available ) {
				_ = new window.JavaStorage(function () {
                    _ = undefined;
                }, 3000);
			}
			timer = undefined;
			...
			timer = setInterval(function () {
				if ( _.isLoaded() ) {
					...
					assert.throws(
						...
						function (err) {
							return err.name === "JavaQuotaExceededError";
						},
						"Testing that JavaStorage throws exception if data " +
	...
	QUnit.test("StorageEvent", function (assert) {
		var iframe, done = assert.async(), counter = 0,
            ...
            checkEvent = function (_event) {
                ...
                if ( _event.storage && 
                     ( _event.storage.type === "JavaStorage" ) ) {
                    switch (counter) {
                        case 0:
                            assert.strictEqual(
                                _event.key, 
		...
		if ( _ ) {
			timer = setInterval(function () {
				if ( _.isLoaded() ) {
                    ...
                    iframe.setAttribute("src", "JavaStorage.html"); 
                    appendChild(
